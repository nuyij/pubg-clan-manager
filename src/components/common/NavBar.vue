<template>
  <nav class="bg-clan-card border-b border-clan-border sticky top-0 z-40">
    <div class="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between gap-3">
      ...
      <!-- 시즌 선택 -->
      <select v-if="season.seasons.length > 1"
          ...
          <button @click="showProfile = true" class="flex items-center gap-2 hover:opacity-80 transition-opacity">
            ...
            <div class="hidden sm:block text-left">
              <div class="text-xs text-clan-text font-body leading-none">
                {{ auth.member?.clan_nickname || auth.discordName || '닉네임 미설정' }}
              </div>
    ...
    <!-- 가입 신청 안내 배너 -->
    <div v-if="auth.isUnregistered" class="bg-blue-900/40 border-t border-blue-700/40 px-4 py-2 text-center text-xs text-blue-300">
      👋 클랜에 가입 신청하려면
      <button @click="handleJoinRequest" :disabled="joining" class="underline ml-1 font-bold text-blue-200">
        {{ joining ? '신청 중...' : '가입 신청하기' }}
      </button>
    </div>

    <!-- 승인 대기 배너 -->
    <div v-else-if="auth.isPending" class="bg-yellow-900/40 border-t border-yellow-700/40 px-4 py-2 text-center text-xs text-yellow-300">
      ⏳ 가입 신청이 접수되었습니다. 관리자 승인을 기다려주세요.
    </div>

    <!-- 닉네임 미설정 배너 -->
    <div v-else-if="auth.needsNickname" class="bg-yellow-900/40 border-t border-yellow-700/40 px-4 py-2 text-center text-xs text-yellow-300">
      ⚠️ 클랜 닉네임을 설정해주세요.
