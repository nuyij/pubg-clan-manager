    ...
    <div class="card max-w-lg w-full p-6 animate-slide-up shadow-gold space-y-5 max-h-[90vh] overflow-y-auto">
      ...
      <div>
        ...
        <div class="mt-2 space-y-1">
          <div class="text-xs text-yellow-500">※ 배그 닉네임 변경 시 반드시 수정해주세요</div>
          <div class="text-xs text-clan-muted bg-clan-surface rounded p-2 border border-clan-border leading-relaxed">
            ⚠️ <span class="text-yellow-400 font-bold">대소문자 구분</span>이 있습니다.<br>
            인게임 닉네임과 <span class="text-clan-gold">정확히 동일하게</span> 입력해주세요.<br>
            예) <span class="font-mono text-clan-text">KSA_yorong</span> ≠ <span class="font-mono text-red-400">KSA_YORONG</span>
          </div>
        </div>
      </div>
...
async function setPrimary(acc) {
  ...
  saving.value = true
  try {
    await supabase.from('member_pubg_accounts').update({ is_primary: false }).eq('member_id', auth.member.id)
