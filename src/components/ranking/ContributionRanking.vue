  ...
  <div class="animate-slide-up">
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-clan-muted text-sm mt-1">클랜원과 함께한 파티 플레이 점수</p>
      </div>
    ...
    <div v-else class="space-y-3">
      ...
      <div class="grid grid-cols-3 gap-3 mb-6">
        <div v-for="item in ranking.contributionRanking.slice(0,3)" :key="item.display_name"
          :class="['card p-4 text-center relative overflow-hidden', topClass(item.rank)]">
          ...
          <!-- 다중 계정 표시 -->
          <div v-if="item.pubg_names?.length > 1" class="text-[10px] text-clan-muted mt-0.5 truncate">
        ...
        <table class="w-full">
          ...
          <tbody>
            <tr v-for="item in ranking.contributionRanking.slice(3)" :key="item.display_name"
              class="border-b border-clan-border/50 hover:bg-clan-surface/50 transition-colors">
              ...
              <td class="px-4 py-3">
                ...
                <div v-if="item.pubg_names?.length > 1" class="text-[10px] text-clan-muted font-mono">
                  {{ item.pubg_names.join(' · ') }}
                </div>
              </td>
